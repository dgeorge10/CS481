...
for char in booktext:
    lower = char.lower()
    if lower in states:
        if lower not in graph: graph[lower] = {}
        for i in range(n+1):
            if booktext[index+i].lower() in states:
                if booktext[index+i].lower() in graph[lower]:
                    graph[lower][booktext[index+i].lower()] += 1
                else:
                    graph[lower][booktext[index+i].lower()] = 1
    index += 1
